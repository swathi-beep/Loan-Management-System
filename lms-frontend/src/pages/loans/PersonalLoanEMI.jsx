...
import {
  User, ArrowLeft, ShieldCheck, Globe, Banknote,
  Clock, CheckCircle2, ChevronDown, FileText, UserCheck,
  Target, Award, Sparkles, Zap
} from "lucide-react";
    ...
    <div className="min-h-screen app-gradient-bg relative overflow-x-hidden text-slate-900 font-sans">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-24">
        ...
        <div className="pt-8">
          <button
            onClick={() => navigate("/")}
            className="flex items-center gap-2 text-slate-900 hover:text-emerald-700 transition-all text-xs font-black uppercase tracking-[0.2em]"
          >
        ...
        <section className="pt-16 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-left space-y-6">
            ...
            >
              ...
              <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">LMS Trust Personal Finance</span>
            </motion.div>
 
            <h1 className="text-5xl md:text-7xl font-serif font-bold text-slate-900 leading-[1.1] tracking-tight">
              Unlock Your <br />
              <span className="text-emerald-600 italic">Next Milestone.</span>
            </h1>
           
            <p className="max-w-lg text-slate-600 text-lg font-medium leading-relaxed">
              Unsecured capital designed for your life's essential moments.
              From home renovations to medical needs, get funds in 24 hours.
            </p>
 
            <div className="flex gap-4">
               <button
                onClick={scrollToCalculator}
                className="px-8 py-4 bg-emerald-600 text-white font-black text-xs uppercase tracking-widest rounded-full shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all flex items-center gap-2"
              >
                Calculate EMI <ChevronDown size={16} />
              </button>
            </div>
          </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-16">
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {["Identity Proof (PAN)", "Address Proof", "3 Months Salary Slips", "6 Months Bank Statement", "Employment Contract", "Passport Photo"].map((text, i) => (
          ...
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-lg font-bold text-slate-900 uppercase tracking-tighter">Eligibility track</h3>
            </div>
            <div className="flex justify-between relative px-2">
              {[
                { icon: <Clock size={16}/>, label: "Check" },
                { icon: <FileText size={16}/>, label: "Upload" },
                { icon: <UserCheck size={16}/>, label: "KYC" },
                { icon: <Award size={16}/>, label: "Payout" }
              ].map((step, i) => (
        ...
        <section ref={calcRef} className="pb-16 scroll-mt-6">
          <div className="bg-gradient-to-tr from-white to-emerald-50/50 border-2 border-emerald-100 rounded-[3rem] p-8 lg:p-12 shadow-[0_40px_100px_-20px_rgba(16,185,129,0.12)]">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
             
              <div className="space-y-8">
                <header>
                  <h2 className="text-3xl font-serif font-bold text-slate-900 leading-none">EMI Estimator</h2>
                  <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mt-2">Plan your monthly budget</p>
                </header>
 
                {[
                  { label: "Desired Amount", value: amount, set: setAmount, min: 50000, max: 3000000, step: 50000, unit: "₹" },
                  { label: "Interest Rate", value: rate, set: setRate, min: 8, max: 24, step: 0.05, unit: "%" },
                  { label: "Repayment Tenure", value: tenure, set: setTenure, min: 1, max: 7, step: 1, unit: "Yrs" },
                ].map((item, idx) => (
                  <div key={idx}>
                    <div className="flex justify-between items-center mb-3">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{item.label}</label>
                      <span className="text-sm font-black text-slate-900 tabular-nums bg-white px-3 py-1 rounded-lg border border-emerald-100 shadow-sm">
                    ...
                    <input
                      ...
                      className="w-full h-1.5 bg-slate-200 rounded-full appearance-none cursor-pointer accent-emerald-600 shadow-sm"
                    />
                  </div>
                ))}
              </div>
              ...
              {/* Result Area */}
              <div className="flex flex-col items-center bg-white rounded-[2.5rem] p-8 border border-emerald-100 shadow-[0_20px_50px_rgba(0,0,0,0.04)]">
                <div className="relative w-32 h-32 mb-6">
                  <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                    <circle cx="50" cy="50" r="35" stroke="#F1F5F9" strokeWidth="10" fill="transparent" />
                    <motion.circle
                      cx="50" cy="50" r="35" stroke="#10b981" strokeWidth="10" fill="transparent"
                ...
                <div className="w-full space-y-2 mb-8">
                  ...
                  <div className="flex justify-between p-3 bg-emerald-600 rounded-xl text-white shadow-xl shadow-emerald-300/40">
